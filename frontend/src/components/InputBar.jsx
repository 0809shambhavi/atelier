...
export default function InputBar({ value, onChange, onSend, loading }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 24px 16px', borderTop:'0.5px solid var(--border)', background:'var(--bg)', flexShrink:0 }}>
      <input type="text" placeholder="Describe your vibe, occasion, or ask anything fashion…" value={value} onChange={e => onChange(e.target.value)} onKeyDown={handleKey} disabled={loading}
        style={{ flex:1, height:44, border:'0.5px solid var(--border-md)', borderRadius:'var(--r-pill)', padding:'0 20px', fontSize:13, background:'var(--surface)', color:'var(--text)', outline:'none', transition:'border-color .15s, box-shadow .15s' }}
      ...
      />
      <button onClick={() => canSend && onSend(value)} disabled={!canSend}
